...
if __name__ == '__main__':
    ...
    for ds in datasets:
        ...
        result = subprocess.run(['./c/origTotientRange', '1', str(ds)],
        ...
        for thread_count in thread_counts:
            ...
            for run in range(3):
                ...
                result = subprocess.run(['./c/totientRange', '1', str(ds), str(thread_count)],
                                        stdout=subprocess.PIPE)
            ...
            out.append(f"Num threads: {thread_count}\n")
            out.append(str(mean(run_times)) + 's\n')

    with open('DPT_coursework_openmp_logs.txt', 'w+') as file:
        for log in out:
            file.write(log)
